...
Description: Displays Linux system memory usage and process resident memory using information from the /proc filesystem. Supports human-readable output and configurable text-based memory graphs.
...
def pids_of_prog(app_name: str) -> list:
    ...
    # Run the pidof command to get all process IDs
    # that belong to the specified program.
    output = os.popen(f"pidof {app_name}").read().strip()

    # If the program is not running, return an empty list.
    if output == "":
        return []

    # Split the process IDs into a list and return them.
    return output.split()
...
def rss_mem_of_pid(proc_id: str) -> int:
    "given a process id, return the Resident memory used"
     ...
     # Store the total Resident Set Size (RSS) memory.
    total_rss = 0
    
    try:

        # Open the smaps file for the specified process.
        with open(f"/proc/{proc_id}/smaps", "r") as smaps:

            # Read each line in the file.
            for line in smaps:

                # Every line starting with "Rss:" contains a portion of the process's resident memory
                if line.startswith("Rss:"):

                    # Split the line and add the memory value to the running total.
                    parts = line.split()
                    total_rss += int(parts[1])

    # The process may end before the file is opened.
    # Return 0 if the file cannot be found.
    except FileNotFoundError:
        return 0 

    # Return the total RSS memory in KiB.
    return total_rss
